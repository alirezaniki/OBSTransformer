...
class DataGenerator(keras.utils.Sequence):
    ...
    def _drop_channel(self, data, snr, rate):
        ...
        data = np.copy(data)
        if np.random.uniform(0, 1) < rate and snr >= 10.0: 
    ...
    def _add_noise(self, data, snr, rate):
        'Randomly add obs noie with a random SNR into waveforms'

        data_noisy = np.empty((data.shape))
        if np.random.uniform(0, 1) < rate and snr>=10.0: 
            rand_noise = random.choice(os.listdir(noisedir))
            np_noise = np.load(os.path.join(noisedir, rand_noise), allow_pickle = True)

            norm_noise = np.zeros(shape = (6000, 3))
            norm_noise[:, 0] = np_noise[:,0] / np.max(np.abs(np_noise[:,0]))
            norm_noise[:, 1] = np_noise[:,1] / np.max(np.abs(np_noise[:,1]))
            norm_noise[:, 2] = np_noise[:,2] / np.max(np.abs(np_noise[:,2]))        

            data_noisy[:, 0] = data[:,0] + np.random.uniform(0.01, 0.15)*norm_noise[:,0]*max(data[:,0])
            data_noisy[:, 1] = data[:,1] + np.random.uniform(0.01, 0.15)*norm_noise[:,1]*max(data[:,1])
            data_noisy[:, 2] = data[:,2] + np.random.uniform(0.01, 0.15)*norm_noise[:,2]*max(data[:,2])
        else:
            data_noisy = data
        return data_noisy   
    ...
    def _add_event(self, data, addp, adds, coda_end, snr, rate): 
        ...
        if addp and adds:
            s_p = adds - addp
            if np.random.uniform(0, 1) < rate and snr>=10.0 and (data.shape[0]-s_p-21-coda_end) > 20:     
                try:
        ...
        for i, ID in enumerate(list_IDs_temp):
            ...
            if dataset.attrs['trace_category'] == 'earthquake_local':
                if self.label_type  == 'gaussian': 
                    ...
                    if sd and sst:
                        if sst+int(0.15*sd) <= self.dim: 
                            y1[i, spt:int(sst+(0.15*sd)), 0] = 1        
                        else:
                    ...
                    if sd and sst+int(0.15*sd) <= self.dim: 
                        y1[i, spt:int(sst+(0.15*sd)), 0] = 1        
                    else:
                        y1[i, spt:self.dim, 0] = 1         
                    if spt: 
...
class PreLoadGenerator(keras.utils.Sequence):
    ...
    def _drop_channel(self, data, snr, rate):
        ...
        data = np.copy(data)
        if np.random.uniform(0, 1) < rate and snr >= 10: 
            c1 = np.random.choice([0, 1])
            c2 = np.random.choice([0, 1])
            c3 = np.random.choice([0, 1])
            if c1 + c2 + c3 > 0:
    ...
    for i, ID in enumerate(list_IDs):
        ...
        if augmentation:                 
            ...
            if dataset.attrs['trace_category'] == 'earthquake_local': 
                ...
                sd = sst - spt      
                if sst+int(0.15*sd) <= dim: 
                    y1[i, spt:int(sst+(0.15*sd)), 0] = 1  
                    y1[len(list_IDs)+i, spt:int(sst+(0.15*sd)), 0] = 1                              
                else:
                    y1[i, spt:dim, 0] = 1
